...
{
    if (p->regs[REG_PPUMASK]&PPUMASK_b) {
        ...
        if (x <= 0xFF) {//绘制可视区域扫描线
            ...
            if (pal) {
                ...
                p->pixels[y][x].c = p->read8(0x3F00 + (cs << 2) + pal) & 0x3F;//真实的系统调色板颜色索引
            } else {
                p->pixels[y][x].priority = PPU_PRIO_NONE;
                p->pixels[y][x].c = p->read8(0x3F00) & 0x3F;//系统默认背景颜色
            }
            if (x < 8 && !(p->regs[REG_PPUMASK] & PPUMASK_m)) {//应该是等于0
                p->pixels[y][x].priority = PPU_PRIO_NONE;
                p->pixels[y][x].pal = 0;
                p->pixels[y][x].c = p->read8(0x3F00) & 0x3F;
            }
        }
    } else {
        if (x <= 0xFF) {
    ...
    if (x > 0xFF) return;
    if (p->regs[REG_PPUMASK]&PPUMASK_s) {
...
{//按照扫描线上的tile为单位获取渲染资源
    const int xscroll = (p->reg_v & 0x1f) << 3 | p->reg_x;
    const int yscroll = (p->reg_v >> 5 & 0x1f) << 3 | p->reg_v >> 12;
    uint16_t curattr = p->read8(PPU_ATTR_ADDR(p->reg_v)), tmpattr = 0;
...
int ppu_step_i(ppu_context *p)
{
    if (p->frame_ticks == 89342) {//时钟周期用完了
        ...
        const int32_t scanline = p->frame_ticks / 341 - 1;//当前扫描线，扫描线从-1开始，Y
        const uint32_t scanline_cycle = p->frame_ticks % 341;//当前扫描线下的第几个tick，X
        if (p->regs[REG_PPUMASK]&(PPUMASK_b|PPUMASK_s) && scanline >= -1 && scanline <= 239) {//可视区域的扫描线
            if (scanline == -1) {
