...
static uint32_t
mmc3_cpuaddr2romoffset(struct cnes_context *av, uint16_t addr)
{
	struct mmc3_context *mc = av->mmc.priv;

	if (addr >= 0xE000 && addr <= 0xFFFF) {
		/* 8KB PRG ROM bank, fixed to the last bank */
		return (uint32_t)((addr - 0xE000) + (av->prg_len - 0x2000));
	}
	...
	if (((mc->banksel >> 6) & 1) == 0) {
		...
		if (addr >= 0x8000 && addr <= 0x9FFF) {
			...
			return (uint32_t)((addr - 0xC000) + (av->prg_len - 0x4000));
		}
	} else {
		...
		if (addr >= 0xC000 && addr <= 0xEFFF) {
			...
			/* 8KB PRG ROM bank, fixed to the second-last last bank */
			return (uint32_t)((addr - 0x8000) + (av->prg_len - 0x4000));
		}
	}
}

static uint8_t
mmc3_cpuread(struct cnes_context *av, uint16_t addr)
{
	struct mmc3_context *mc = av->mmc.priv;
...
static void
mmc3_cpuwrite(struct cnes_context *av, uint16_t addr, uint8_t val)
{
	struct mmc3_context *mc = av->mmc.priv;
...
static uint32_t
mmc3_ppuaddr2romoffset(struct cnes_context *av, uint16_t addr)
{
	struct mmc3_context *mc = av->mmc.priv;
...
static uint8_t
mmc3_ppuread(struct cnes_context *av, uint16_t addr)
{
	struct mmc3_context *mc = av->mmc.priv;
	//uint8_t val;
...
static void
mmc3_ppuwrite(struct cnes_context *av, uint16_t addr, uint8_t val)
{
	struct mmc3_context *mc = av->mmc.priv;
...
static int
mmc3_init(mapper_context *m, const uint8_t *data, size_t datalen)
{
